...
if __name__ == '__main__':
    ...
    parser = ArgumentParser(description="Formats raw results from the individual enzyme annotation tools.")
    ...
    # If the folder does not exist, create it, and proceed as usual.
    # If not, verify that the raw results haven't already been formatted. If so, skip this step.
    if not os.path.isdir(main_folder_with_results):
        os.mkdir(main_folder_with_results)
    elif os.path.isdir(main_folder_with_results + "/Readable_results"):
        status_writer.write("Step_3: " + str(datetime.datetime.now()) + ": Architect has detected that raw results have previously been formatted.\n")
        status_writer.close()
        exit()
