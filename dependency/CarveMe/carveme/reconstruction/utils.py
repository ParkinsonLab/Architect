# Nirvana Nursimulu modified various parts of this code for Architect.
...
def load_reaction_scores(filename, sep='\t', reaction_col='reaction', score_col='normalized_score'):

    data = pd.read_csv(filename, sep=sep)
    score_db = data[[reaction_col, score_col]].groupby(reaction_col).agg(lambda x: float(x)) 
	
    return score_db[score_col].to_dict()
...
def medium_to_constraints(model, compounds, max_uptake=1000, inplace=False, verbose=False, exchange_format=None):
...
def write_gap_filling_solutions(filename, results):
    with open(filename, 'w') as writer:
        for i, result in enumerate(results):
            writer.write("Sol_" + str(i) + "\t")
            for rxn in result:
                writer.write(rxn + "\t")
            writer.write("\n")